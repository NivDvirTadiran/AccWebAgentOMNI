                                        ...
                                        char *  agent_name)
{
	...
	//
	m_SbcWorkThread->m_sea_reply.RemoveInsertToAgents_device_Map(ADI->m_Extenstion,ADI->m_Agent_id);
	//
	...
	if (m_SbcWorkThread->m_TableChanged == true)
	{
		m_SbcWorkThread->GetAllTables(true);
	}
	GetAgentGroups(ADI);
                                     ...
                                     char *   agent_session_id)
{
	...
	if (cause != 9) // diff agent on same ext
	{
		m_SbcWorkThread->m_sea_reply.RemoveDeviceMonitor(m_SbcWorkThread->m_sea_reply.GetRequestId(),ext,0);
