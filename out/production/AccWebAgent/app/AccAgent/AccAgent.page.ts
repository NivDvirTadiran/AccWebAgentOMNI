...
import {HostListener, Component, OnInit, OnDestroy, ElementRef, AfterViewInit, Renderer2} from '@angular/core';
...
import { AccButtons, accbutton} from './AccBtnsDef'
import { UserStatusModel,COS, oneTab,LoginUser, Agent, AccCallState, OneCall, Key_Desc, AgentStatus, CallTypes, CallTypesText,
    QUEUED_ACD_GROUP,barchart, stackedcolchart } from './data-model.interface';
import { TranslateService } from '@ngx-translate/core';
import * as CanvasJS from '../../assets/js/canvasjs-commercial-3.2.6/canvasjs.min.js';
...
@Component({
    selector: 'Acc-agent',
    templateUrl: './AccAgent.page.html',
    styleUrls: ['./AccAgent.page.scss']
})

export class AccAgentPage implements OnInit, OnDestroy, AfterViewInit {
        ...
        for (let g = 0; g < this.setupGroups.length; ++g) {
            ...
            for (let s = 0; s < this.setupGroups[g].setup.length; ++s) {
                try {
                    ...
                    stpini = this.AAC.etasIni.AgentSetup[g].setup[s];
   ...
   // ]=========================== [SelctedSetupChange ]===========================
    SelctedSetupChange(evt) {
        for (let g = 0; g < this.setupGroups.length; ++g) {
            ...
            for (let s = 0; s < this.setupGroups[g].setup.length; ++s) {
                try {
                    stpini = this.AAC.etasIni.AgentSetup.Options[g].Setup[s];
